...
const Notice = (prop) => {
  ...
  <div className="relative flex justify-between items-center w-full">
    ...
    {open ? (
      <button
        className="absolute right-2 flex justify-center items-center border-2 border-purple-500 px-3 py-2 rounded text-purple-500 hover:bg-purple-100"
        onClick={openHandler}
      >
        <span className="mr-2">
          <BiArrowBack className="text-purple-500" />
        </span>
        Close
      </button>
    ) : (
      ...
      >
        Add Notice
        <span className="ml-2">
          <IoAddOutline className="text-purple-500 text-xl" />
        </span>
      </button>
    )}
  </div>
    ...
    <div className="mt-8 w-full">
      {notice &&
        notice.map((item, index) => (
          <div
            key={item._id}
            className="border-indigo-500 border-2 w-full rounded-md shadow-sm py-4 px-6 mb-4 relative bg-indigo-100"
          >
            <div className="absolute flex justify-center items-center right-4 bottom-3">
              ...
              <span
                className="text-2xl ml-2 cursor-pointer hover:text-indigo-500"
            ...
            >
              {item.title}
              {item.link && (
                <span className="text-2xl group-hover:text-indigo-600 ml-1">
                  <IoMdLink />
                </span>
              )}
            </p>
...
{open && (
  <form className="mt-8 max-w-xl w-full mx-auto bg-white border rounded-2xl p-8 shadow-md">
    ...
    <div className="w-full mt-4">
      ...
      {!file && (
        <label
          htmlFor="upload"
          className="px-4 bg-blue-50 py-2 rounded-sm w-full border text-base w-[90%] mt-1 flex justify-center items-center cursor-pointer"
        >
          Upload Material
          <span className="ml-2">
            <FiUpload />
  ...
  className="mt-2 bg-blue-600 hover:bg-blue-700 text-white text-center items-center font-semibold px-6 py-2 rounded-xl shadow transition duration-200"
    >
